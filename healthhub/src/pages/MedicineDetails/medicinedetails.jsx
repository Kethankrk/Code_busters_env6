    ...
    <div className='px-4 flex flex-col items-center pb-28'>
      <div className='w-full flex  flex-col p-5 shadow-[rgba(50,50,93,0.25)_0px_6px_12px_-2px,_rgba(0,0,0,0.3)_0px_3px_7px_-3px] mt-5 rounded-3xl'>
        <div className='flex items-center gap-3'>
          ...
          <div className=''>
            <h1 className='font-bold text-2xl'>Medicine Details</h1>
            <p> Find the details of your medicine </p>
          </div>
        </div>
        ...
        <form
          onSubmit={submit}
          className='flex  shadow-[0_3px_10px_rgb(0,0,0,0.2)] px-5 py-3 rounded-full gap-1 mt-5'
        >
          <input
            type='text'
            required
      ...
      {result != null ? (
        <div className='w-full flex flex-col p-5 shadow-[rgba(50,50,93,0.25)_0px_6px_12px_-2px,_rgba(0,0,0,0.3)_0px_3px_7px_-3px] mt-5 rounded-3xl'>
          <>
            <h1 className='font-bold text-lg mb-3 text-center'>Result </h1>
            <span>
              <span className='font-bold'>USES : </span>{' '}
              {result.indications_and_usage.map((e) => (
                <p>{e.replace('•', '')}</p>
              ))}
            </span>
            {result.purpose != null ? (
            ...
            {result.active_ingredient != null ? (
              <span className='pt-5'>
                <span className='font-bold'> Ingredient : </span>{' '}
                {result.active_ingredient.map((e) => (
                  <p>{e.replace('•', '')}</p>
                ))}
              </span>
            ) : (
            ...
            <span className='pt-5'>
              <span className='font-bold'> Dosage and administration : </span>{' '}
              {result.dosage_and_administration.map((e) => (
                <p>{e.replace('•', '')}</p>
              ))}
            </span>
